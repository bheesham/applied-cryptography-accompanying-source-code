...
#ifdef TEST_MPEG
  #undef BUFFER_SIZE
  #define BUFFER_SIZE		1300000		/* Large buffer for cat MPEG */
#endif /* TEST_MPEG */
...
int testCert( void )
	...
	status = cryptImportCert( certBuffer, &cryptCert );
	if( cryptStatusError( status ) )
		{
...
static const CERT_DATA setCertData[] = {
	...
	{ CRYPT_CERTINFO_SET_CERTIFICATETYPE, IS_NUMERIC, TRUE },
...
int testCRL( void )
	...
	/* Make sure we can read what we created */
	status = cryptImportCert( certBuffer, &cryptCRL );
	if( cryptStatusError( status ) )
		{
		printf( "cryptImportCert() failed with error code %d, line %d\n",
				status, __LINE__ );
...
int testCertChain( void )
	...
	/* Make sure we can read what we created */
	status = cryptImportCert( certBuffer, &cryptCertChain );
	if( cryptStatusError( status ) )
		{
		printf( "cryptImportCert() failed with error code %d, line %d\n",
				status, __LINE__ );
	...
	/* Display info on each cert in the chain.  This uses the cursor
	   ...
	   the root */
	count = 0;
	cryptAddCertComponentNumeric( cryptCertChain, CRYPT_CERTINFO_CURRENT_CERTIFICATE,
								  CRYPT_CURSOR_FIRST );
	do
		{
		printf( "Certificate %d\n-------------\n", count++ );
		printCertInfo( cryptCertChain );
		putchar( '\n' );
		}
	while( cryptAddCertComponentNumeric( cryptCertChain,
...
int testCertImport( void )
	...
	status = cryptImportCert( buffer, &cryptCert );
	if( status == CRYPT_NOALGO || status == CRYPT_BADDATA )
		{
...
int testCertChainImport( void )
	...
	status = cryptImportCert( buffer, &cryptCertChain );
	if( status == CRYPT_NOALGO || status == CRYPT_BADDATA )
		{
...
static int createCertRequest( void *certRequest )
	...
	if( cryptStatusOK( status ) )
		status = cryptAddCertComponentString( cryptCert,
					CRYPT_CERTINFO_COUNTRYNAME, "NZ", 2 );
	if( cryptStatusOK( status ) )
		status = cryptAddCertComponentString( cryptCert,
					CRYPT_CERTINFO_ORGANIZATIONNAME,
					"Dave's Wetaburgers", 18 );
	if( cryptStatusOK( status ) )
		status = cryptAddCertComponentString( cryptCert,
	...
	if( cryptStatusOK( status ) )
		status = cryptAddCertComponentString( cryptCert,
					CRYPT_CERTINFO_COMMONNAME, "Dave Smith", 10 );
	if( cryptStatusOK( status ) )
		status = cryptAddCertComponentString( cryptCert,
					CRYPT_CERTINFO_RFC822NAME,
					"dave@wetas-r-us.com", 19 );
	if( cryptStatusOK( status ) )
		status = cryptAddCertComponentString( cryptCert,
					CRYPT_CERTINFO_UNIFORMRESOURCEIDENTIFIER,
					"http://www.wetas-r-us.com", 25 );
#ifdef TEST_MPEG
	{
	FILE *filePtr;
	BYTE *buffer = malloc( 1300000L );
	int size;

	memcpy( buffer, "\x04\x83\x12\xA1\x32", 5 );	/* OCTET STRING */
	if( ( filePtr = fopen( "e:\\spool\\cat.mpg", "rb" ) ) == NULL )
		return( FALSE );
	size = fread( buffer + 5, 1, 1300000 - 5, filePtr ) + 5;
	fclose( filePtr );
	status = cryptAddCertComponentString( cryptCert,
				CRYPT_CERTINFO_OTHERNAME_TYPEID,
				"1 3 6 1 4 1 3029 42 11172 1", 27 );	/* MPEG-1 */
	status = cryptAddCertComponentString( cryptCert,
...
static int createCertificate( void *certificate, const void *certRequest,
	...
	status = cryptImportCert( certRequest, &cryptCertRequest );
	if( cryptStatusOK( status ) )
	...
	if( cryptStatusOK( status ) )
		{
		/* Turn on MS-compatible broken certificate generation */
		cryptSetOptionNumeric( CRYPT_OPTION_CERT_ENCODE_CRITICAL, FALSE );
/*		status = cryptAddCertComponentNumeric( cryptCert,
					CRYPT_CERTINFO_EXTKEY_MS_SERVERGATEDCRYPTO, CRYPT_UNUSED );
*/
		}
#endif /* TEST_MSIE */
	if( cryptStatusOK( status ) )
	...
	if( cryptStatusOK( status ) )
		status = cryptExportCert( certificate, &length,
								  CRYPT_CERTFORMAT_TEXT_CERTCHAIN, cryptCert );
#else
	if( cryptStatusOK( status ) )
		status = cryptExportCert( certificate, &length,
								  CRYPT_CERTFORMAT_CERTIFICATE, cryptCert );
...
int testCertProcess( void )
	...
	puts( "Testing certificate processing..." );
	...
	if( ( filePtr = fopen( "e:\\tmp\\certreq.der", "rb" ) ) == NULL )
		return( FALSE );
	fread( certBuffer, 1, BUFFER_SIZE, filePtr );
	fclose( filePtr );
	}
#else
	/* Create the certification request */
	status = createCertRequest( certBuffer );
	if( cryptStatusError( status ) )
		{
		printf( "Certification request creation failed with error code %d, "
				"line %d\n", status, __LINE__ );
		return( FALSE );
		}
	debugDump( "prcertrq", certBuffer, status );
#endif /* TEST_MSIE */

	/* Create a certificate from the certification request */
	status = createCertificate( certBuffer, certBuffer, cryptCAKey );
	if( cryptStatusError( status ) )
		{
	...
	debugDump( "prcert", certBuffer, status );
	...
	cryptDestroyCert( cryptCert );
	cryptDestroyContext( cryptCAKey );
	puts( "Certificate processing succeeded.\n" );
	return( TRUE );
	}
